 ...
 */
export function normalizeAccess(role: RbacRole, access?: PageAccessKey[]): PageAccessKey[] {
  const allowed = role === 'admin' ? ADMIN_PAGE_ACCESS : STAFF_PAGE_ACCESS;
  const candidate = access && access.length ? access : allowed;
  return Array.from(new Set(candidate)).filter((entry): entry is PageAccessKey => allowed.includes(entry));
}
