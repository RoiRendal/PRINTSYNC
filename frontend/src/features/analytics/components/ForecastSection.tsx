...
export function ForecastSection({
  ...
  return (
    <SectionCard
      ...
      controls={<div className="space-y-2"><div className="surface-segmented flex rounded-full p-1"><button type="button" onClick={() => setForecastMetric('income')} aria-pressed={forecastMetric === 'income'} className={cn('mat-focus h-8 cursor-pointer rounded-full px-3 text-[10px] font-bold uppercase tracking-[0.16em] transition-colors', forecastMetric === 'income' ? 'mat-selected-segment bg-segment-green text-white' : 'ambient amb-elevation-0 mat-press text-macos-text-muted hover:text-macos-text dark:hover:text-zinc-100')}>Income</button><button type="button" onClick={() => setForecastMetric('expenses')} aria-pressed={forecastMetric === 'expenses'} className={cn('mat-focus h-8 cursor-pointer rounded-full px-3 text-[10px] font-bold uppercase tracking-[0.16em] transition-colors', forecastMetric === 'expenses' ? 'mat-selected-segment bg-segment-orange text-white' : 'ambient amb-elevation-0 mat-press text-macos-text-muted hover:text-macos-text dark:hover:text-zinc-100')}>Expenses</button></div><PeriodSelector value={forecastPeriod} onChange={onForecastPeriodChange} prefix="forecast" /></div>}
    >
      {isLoading ? <LoadingState label="Loading inventory forecast" /> : error ? <ErrorState message={error} /> : !inventoryForecast || inventoryForecast.items.length === 0 ? <p className="text-xs text-macos-text-muted">No inventory forecast data available for this period.</p> : (
        <>
