...
export function ForecastSection({
  ...
  return (
    <SectionCard
      ...
      controls={<div className="space-y-2"><div className="flex rounded-full border p-1"><button type="button" onClick={() => setForecastMetric('income')} className={cn('h-8 cursor-pointer rounded-full px-3 text-[10px] font-bold', forecastMetric === 'income' ? 'bg-macos-green text-white' : 'text-macos-text-muted hover:bg-[var(--app-state-hover)] dark:text-zinc-400 dark:hover:bg-[#414143]')}>Income</button><button type="button" onClick={() => setForecastMetric('expenses')} className={cn('h-8 cursor-pointer rounded-full px-3 text-[10px] font-bold', forecastMetric === 'expenses' ? 'bg-macos-orange text-white' : 'text-macos-text-muted hover:bg-[var(--app-state-hover)] dark:text-zinc-400 dark:hover:bg-[#414143]')}>Expenses</button></div><PeriodSelector value={forecastPeriod} onChange={onForecastPeriodChange} prefix="forecast" /></div>}
    >
